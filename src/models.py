...
def build_module(task, model, d_sent, d_emb, vocab, embedder, args):
    ...
    if isinstance(task, SingleClassificationTask):
        ...
        setattr(model, '%s_singlepooler' % task.name, module)
        ...
        task.img_encoder = CNNEncoder(model_name='resnet', path=task.path, dataset=task.dataset)
        pooler = build_image_sent_module(task, d_sent, task_params)
...
class MultiTaskModel(nn.Module):
    ...
    def forward(self, task, batch, predict=False):
        ...
        if isinstance(task, SingleClassificationTask):
            ...
            #out = self._grounded_ranking_forward(batch, task, predict)
            out = self._grounded_contrastive_forward(batch, task, predict)
    ...
    def _grounded_contrastive_forward(self, batch, task, predict):
    ...
    def _grounded_ranking_forward(self, batch, task, predict):
        ''' Contrastive/ranking
        ...
        out = {}
        sent_emb, sent_mask = self.sent_encoder(batch['input1'])
        batch_size = get_batch_size(batch); out['n_exs'] = batch_size
        sent_pooler = self._get_classifier(task) 
        sent_rep = sent_pooler(sent_emb, sent_mask)

        idxs = batch['ids'].cpu().squeeze(-1).data.numpy().tolist()
        img_seq = [torch.tensor(task.img_encoder.forward(int(idxs[i]))[0], dtype=torch.float32).cuda() for i in range(len(idxs))]

        cos_sim = torch.mm(F.normalize(sent_rep, 2, 1),
                           torch.transpose(F.normalize(torch.stack(img_seq), 2, 1), 0,1))

        labels = torch.eye(batch_size).cuda()

        matmul = cos_sim  * batch_size
        matmul_t = torch.transpose(matmul, 0, 1)

        for i in range(batch_size):
            for j in range(batch_size):
                if (i == j): continue
                matmul[i] = matmul[i].sub(cos_sim[j])
                matmul_t[i] = matmul_t[i].sub(cos_sim[j])
        
        mat = (matmul + matmul_t) / (2*batch_size)
        scores = F.softmax(mat, dim=1)

        preds = torch.diag(mat)
        true = torch.tensor([i for i in range(batch_size)]).cuda()
        pred_labels = torch.argmax(scores, dim=1)
        preds = (pred_labels == true)

        total_correct = torch.sum(preds)
        batch_acc = total_correct.item()/len(true)
    
        mat = torch.mul(mat, torch.eye(batch_size, dtype=torch.float32).cuda())

        out['loss'] = F.l1_loss(mat, labels)
